 ...
 * @version 1.0
 */
public class QuadriliteralAugmentedNomenGerundConjugator2 {

    private static QuadriliteralAugmentedNomenGerundConjugator2 instance = new QuadriliteralAugmentedNomenGerundConjugator2();
    private Map gerundClassMap = new HashMap();

    private int[] indexArray = {2, 4, 6, 8, 10, 12, 14, 16, 18};

    private QuadriliteralAugmentedNomenGerundConjugator2() {
        for (int i = 1; i <= 3; i++) {
            String gerundClassName = "sarf.gerund.quadrilateral.augmented.nomen.pattern.GerundPattern" + i;
            try {
    ...
    public static QuadriliteralAugmentedNomenGerundConjugator2 getInstance() {
        return instance;
    }

    public List createGerundList(AugmentedQuadriliteralRoot root, int formulaNo) {
        String gerundPatternClassName = "sarf.gerund.quadrilateral.augmented.nomen.pattern.GerundPattern" + formulaNo;
        ...
        for (int i = 0; i < indexArray.length; i++) {
            ...
            try {
//                sarf.gerund.quadrilateral.augmented.nomen.QuadriliteralAugmentedNomenGerund g= Class.forName(gerundPatternClassName).getConstructors()[].newInstance(parameters);
                QuadriliteralAugmentedNomenGerund gerund = (QuadriliteralAugmentedNomenGerund) Class.forName(gerundPatternClassName).getConstructors()[1].newInstance(parameters);
    ...
    /**
     * the key in the map are the gerund pattern, the value are 18 string can be
     * diplayed on the NounConjugationUI
     *
